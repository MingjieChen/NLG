...
class LSTM_decoder:
    def __init__(self,sequence_length,num_words,index2words,memory,h0):
        ...
        #self.generations = tf.reshape(self.gen_x.stack(), [batch_size, self.batch_len])
        self.generations = tf.transpose(self.gen_x.stack())
        #self.gen_prob = tf.reshape(self.gen_o.concat(),[batch_size,self.batch_len,self.num_words])
        self.gen_prob = tf.transpose(self.gen_o.stack(),perm=[1,0,2])
        ...
        self.opt = tf.train.AdamOptimizer(learning_rate=5e-4)
